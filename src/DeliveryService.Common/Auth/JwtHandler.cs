...
namespace DeliveryService.Common.Auth
{
    public class JwtHandler : IJwtHandler
    {
        ...
        public JsonWebToken Create(Guid userId)
        {
            ...
            var expires = nowDate.AddMinutes(_options.ExpiryMinutes);
            var centuryBegin = new DateTime(1970, 1, 1).ToUniversalTime();
            var exp = (long)(new TimeSpan(expires.Ticks - centuryBegin.Ticks).TotalSeconds);
            var now = (long)(new TimeSpan(nowDate.Ticks - centuryBegin.Ticks).TotalSeconds);
            ...
            var toker = _jwtSecurityTokenHandler.WriteToken(jwt);
